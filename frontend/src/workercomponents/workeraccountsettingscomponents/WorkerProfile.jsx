...
export default function WorkerProfile() {
  ...
  async function uploadAvatar(file) {
    try {
      ...
      if (url) { localStorage.setItem("workerAvatarUrl", url); window.dispatchEvent(new CustomEvent("worker-avatar-updated", { detail: { url } })); }
      return url;
    } catch { return ""; }
  }
  ...
  async function removeAvatarServer() {
    try {
      ...
      const url = data?.avatar_url || "";
      if (url === "" || url == null) { localStorage.removeItem("workerAvatarUrl"); window.dispatchEvent(new CustomEvent("worker-avatar-updated", { detail: { url: "" } })); }
      return true;
    } catch { return false; }
  }
