...
const hasAny=Object.keys(dbPatch).length>0;let before=null;try{before=await clientModel.getByAuthUid(s.auth_uid||null,{db:req.supabaseUser})}catch{}if(!before&&s.email){try{before=await clientModel.getByEmail(s.email,{db:req.supabaseUser})}catch{}}const uid=before?.auth_uid||s.auth_uid;if(hasAny){await clientModel.updateClientProfile(uid,dbPatch,{db:req.supabaseUser});if("first_name"in patch||"last_name"in patch){const meta={};if("first_name"in patch)meta.first_name=patch.first_name||"";if("last_name"in patch)meta.last_name=patch.last_name||"";await clientModel.updateAuthUserMeta(uid,meta,{dbAuth:req.supabaseUser})}const changes=[];if(("first_name"in patch)||("last_name"in patch))changes.push("name");if("phone"in patch)changes.push("contact number");if("facebook"in patch)changes.push("facebook");if("instagram"in patch)changes.push("instagram");if("date_of_birth"in patch)changes.push("date of birth");const title="Profile updated successfully";const message=changes.length?`Updated ${changes.join(", ")}.`:"Your details are now up to date.";await notifModel.create({auth_uid:uid,role:"client",title,message})}const payload=await clientModel.getClientAccountProfile({auth_uid:uid,email:s.email},{db:req.supabaseUser});return res.status(200).json(payload)}catch{return res.status(400).json({message:"Failed to update profile"})}};
