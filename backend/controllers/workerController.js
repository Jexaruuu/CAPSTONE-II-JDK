...
const registerWorker = async (req, res) => {
  ...
  try {
    ...
    if (!first_name || !last_name || !sex || !email_address || !password) return res.status(400).json({ message: 'Missing required fields' });
    ...
    const { user: authUser, error: authError } = await createConfirmedUser(email_address, password, { first_name, last_name, sex, role: 'worker', is_agreed_to_terms: true, agreed_at });
    ...
    try {
      await workerModel.createWorker(authUser.id, first_name, last_name, sex, email_address, password, true, agreed_at);
    } catch (dbErr) {
