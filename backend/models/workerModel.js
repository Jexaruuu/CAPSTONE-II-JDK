...
function extFrom(ct){if(ct==="image/png")return"png";if(ct==="image/webp")return"webp";if(ct==="image/jpeg"||ct==="image/jpg")return"jpg";return"bin"}
...
const createWorker=async(auth_uid,firstName,lastName,sex,email,password,isAgreedToTerms,agreedAt)=>{const{data,error}=await supabaseAdmin.from("user_worker").insert([{auth_uid,first_name:firstName,last_name:lastName,sex,email_address:String(email||"").trim().toLowerCase(),password,is_agreed_to_terms:isAgreedToTerms,agreed_at:agreedAt,contact_number:null,social_facebook:null,social_instagram:null,created_at:new Date().toISOString()}]);if(error)throw error;return data};
const checkEmailExistence=async(email)=>{const e=String(email||"").trim().toLowerCase();const{data,error}=await supabaseAdmin.from("user_worker").select("*").ilike("email_address",e);if(error)throw error;return data};
const checkEmailExistenceAcrossAllUsers=async(email)=>{const e=String(email||"").trim().toLowerCase();const{data:wd,error:we}=await supabaseAdmin.from("user_worker").select("*").ilike("email_address",e);if(we)throw we;const{data:cd,error:ce}=await supabaseAdmin.from("user_client").select("*").ilike("email_address",e);if(ce)throw ce;return[...(wd||[]),...(cd||[])]};
const getByAuthUid=async(auth_uid)=>{const{data,error}=await supabaseAdmin.from("user_worker").select("*").eq("auth_uid",auth_uid).limit(1);if(error)throw error;return data&&data[0]?data[0]:null};
const getWorkerAccountProfile=async({auth_uid,email})=>{const row=await getByAuthUid(auth_uid);const user=await getAuthUserById(row?.auth_uid||auth_uid);const created_at=row?.created_at||user?.created_at||null;const dob=row?.date_of_birth||user?.user_metadata?.date_of_birth||null;const age=row?.age!=null?row.age:computeAge(dob);return{first_name:row?.first_name||user?.user_metadata?.first_name||"",last_name:row?.last_name||user?.user_metadata?.last_name||"",email_address:row?.email_address||user?.email||email||"",sex:row?.sex||user?.user_metadata?.sex||"",avatar_url:row?.worker_avatar||user?.user_metadata?.avatar_url||"",phone:row?.contact_number??row?.phone??"",facebook:row?.social_facebook??row?.facebook??"",instagram:row?.social_instagram??row?.instagram??"",auth_uid:row?.auth_uid||auth_uid||"",created_at,date_of_birth:dob,age}};
const uploadWorkerAvatarDataUrl=async(auth_uid,data_url)=>{const parsed=parseDataUrl(data_url);if(!parsed)throw new Error("Invalid image");await ensureStorageBucket(WORKER_BUCKET,true);const key=`workers/${auth_uid||"unknown"}/${Date.now()}.${extFrom(parsed.contentType)}`;const buf=parsed.buf;const{error:upErr}=await supabaseAdmin.storage.from(WORKER_BUCKET).upload(key,buf,{contentType:parsed.contentType,upsert:true});if(upErr)throw upErr;const{data:pub}=supabaseAdmin.storage.from(WORKER_BUCKET).getPublicUrl(key);return pub?.publicUrl||""};
const updateAvatarUrl=async(auth_uid,avatar_url)=>{const{error}=await supabaseAdmin.from("user_worker").update({worker_avatar:avatar_url}).eq("auth_uid",auth_uid);if(error)throw error;return true};
const clearWorkerAvatar=async(auth_uid)=>{const{error}=await supabaseAdmin.from("user_worker").update({worker_avatar:null}).eq("auth_uid",auth_uid);if(error)throw error;return true};
const updateAuthUserAvatarMeta=async(auth_uid,avatar_url)=>{const user=await getAuthUserById(auth_uid);const meta=user?.user_metadata||{};const next={...meta,avatar_url:avatar_url||null};const{error}=await supabaseAdmin.auth.admin.updateUserById(auth_uid,{user_metadata:next});if(error)throw error;return true};
const updatePassword=async(auth_uid,password)=>{const{error}=await supabaseAdmin.from("user_worker").update({password}).eq("auth_uid",auth_uid);if(error)throw error;return true};
...
const updateWorkerProfile=async(auth_uid,patch)=>{const{data,error}=await supabaseAdmin.from("user_worker").update(patch).eq("auth_uid",auth_uid).select("*").limit(1);if(error)throw error;return data&&data[0]?data[0]:null};
