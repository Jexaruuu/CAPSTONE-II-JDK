...
function filenameFromUrl(u){try{const x=new URL(String(u));const seg=x.pathname.split("/").filter(Boolean);return seg[seg.length-1]||null}catch{const s=String(u||"");const i=s.lastIndexOf("/");return i>-1?s.slice(i+1):s}}
function publicUrlFromFilename(uid,fn){if(!uid||!fn)return"";const key=`clients/${uid}/${fn}`;const{data}=supabaseAdmin.storage.from(CLIENT_BUCKET).getPublicUrl(key);return data?.publicUrl||""}

const getClientAccountProfile=async({auth_uid,email})=>{const row=auth_uid?await getByAuthUid(auth_uid):await getByEmail(email);const user=await getAuthUserById(row?.auth_uid||auth_uid);const created_at=row?.created_at||user?.created_at||null;const dob=row?.date_of_birth||user?.user_metadata?.date_of_birth||null;const age=row?.age!=null?row.age:computeAge(dob);let avatar_url=row?.client_avatar||user?.user_metadata?.avatar_url||"";if(avatar_url&&!/^https?:\/\//i.test(avatar_url))avatar_url=publicUrlFromFilename(row?.auth_uid||auth_uid,avatar_url);return{first_name:row?.first_name||user?.user_metadata?.first_name||"",last_name:row?.last_name||user?.user_metadata?.last_name||"",email_address:row?.email_address||user?.email||email||"",sex:row?.sex||user?.user_metadata?.sex||"",avatar_url:avatar_url||"",phone:row?.contact_number??row?.phone??"",facebook:row?.social_facebook??row?.facebook??"",instagram:row?.social_instagram??row?.instagram??"",auth_uid:row?.auth_uid||(user?.id??""),created_at,date_of_birth:dob,age}};

const uploadClientAvatarDataUrl=async(auth_uid,data_url)=>{const parsed=parseDataUrl(data_url);if(!parsed)throw new Error("Invalid image");await ensureStorageBucket(CLIENT_BUCKET,true);const key=`clients/${auth_uid||"unknown"}/${Date.now()}.${extFrom(parsed.contentType)}`;const{error:upErr}=await supabaseAdmin.storage.from(CLIENT_BUCKET).upload(key,parsed.buf,{contentType:parsed.contentType,upsert:true});if(upErr)throw upErr;const{data:pub}=supabaseAdmin.storage.from(CLIENT_BUCKET).getPublicUrl(key);return pub?.publicUrl||""};

const updateAvatarUrl=async(auth_uid,avatar_value)=>{let filename=null;if(avatar_value){if(/^https?:\/\//i.test(String(avatar_value)))filename=filenameFromUrl(avatar_value);else if(String(avatar_value).includes("/")){const parts=String(avatar_value).split("/").filter(Boolean);filename=parts[parts.length-1]}else filename=String(avatar_value)}const{error}=await supabaseAdmin.from("user_client").update({client_avatar:filename}).eq("auth_uid",auth_uid);if(error)throw error;return true};

const clearClientAvatar=async(auth_uid)=>{const{error}=await supabaseAdmin.from("user_client").update({client_avatar:null}).eq("auth_uid",auth_uid);if(error)throw error;return true};
