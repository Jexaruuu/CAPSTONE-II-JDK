...
const getClientAccountProfile=async({auth_uid,email},opts={})=>{
  const db=opts.db||supabase;
  let row=null;try{row=auth_uid?await getByAuthUid(auth_uid,{db}):await getByEmail(email,{db})}catch{}
  ...
  const rawDob=row?.date_of_birth||user?.user_metadata?.date_of_birth||null;
  const dobDate=parseDobToDate(rawDob);
  const dobYMD=dobDate?toYMD(dobDate):(rawDob&&/^\d{4}-\d{2}-\d{2}/.test(String(rawDob))?String(rawDob).slice(0,10):null);
  const age=row?.age!=null?row.age:computeAge(dobYMD||rawDob);
...
const updatePassword=async(auth_uid,password,opts={})=>{const db=opts.db||supabaseAdmin;const{error}=await db.from("user_client").update({password}).eq("auth_uid",auth_uid);if(error)throw error;return true};
...
const updateClientProfile=async(auth_uid,patch,opts={})=>{const db=opts.db||supabaseAdmin;const{data,error}=await db.from("user_client").update(patch).eq("auth_uid",auth_uid).select("*").limit(1);if(error)throw error;return data&&data[0]?data[0]:null};
const updateAuthUserMeta=async(auth_uid,patch,opts={})=>{
  ...
  const user=await getAuthUserById(auth_uid);const base=user?.user_metadata||{};const next={...base,...patch};const{error}=await supabaseAdmin.auth.admin.updateUserById(auth_uid,{user_metadata:next});if(error)throw error;return true
};
