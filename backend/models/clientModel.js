...
const getClientAccountProfile=async({auth_uid,email})=>{const row=await getByAuthUid(auth_uid);const user=await getAuthUserById(row?.auth_uid||auth_uid);const created_at=row?.created_at||user?.created_at||null;const dob=row?.date_of_birth||user?.user_metadata?.date_of_birth||null;const age=row?.age!=null?row.age:computeAge(dob);return{first_name:row?.first_name||user?.user_metadata?.first_name||"",last_name:row?.last_name||user?.user_metadata?.last_name||"",email_address:row?.email_address||user?.email||email||"",sex:row?.sex||user?.user_metadata?.sex||"",avatar_url:row?.client_avatar||user?.user_metadata?.avatar_url||"",phone:row?.contact_number??row?.phone??"",facebook:row?.social_facebook??row?.facebook??"",instagram:row?.social_instagram??row?.instagram??"",auth_uid:row?.auth_uid||auth_uid||"",created_at,date_of_birth:dob,age}};
const uploadClientAvatarDataUrl=async(auth_uid,data_url)=>{const parsed=parseDataUrl(data_url);if(!parsed)throw new Error("Invalid image");await ensureStorageBucket(CLIENT_BUCKET,true);const key=`clients/${auth_uid||"unknown"}/${Date.now()}.${extFrom(parsed.contentType)}`;const buf=parsed.buf;const{error:upErr}=await supabaseAdmin.storage.from(CLIENT_BUCKET).upload(key,buf,{contentType:parsed.contentType,upsert:true});if(upErr)throw upErr;const{data:pub}=supabaseAdmin.storage.from(CLIENT_BUCKET).getPublicUrl(key);return pub?.publicUrl||""};
const updateAvatarUrl=async(auth_uid,avatar_url)=>{const{error}=await supabaseAdmin.from("user_client").update({client_avatar:avatar_url}).eq("auth_uid",auth_uid);if(error)throw error;return true};
